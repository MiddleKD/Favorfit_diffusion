    ...
    for epoch in range(args.epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                params_to_clip = []
                if args.clip == True:
                    params_to_clip += list(clip.parameters())
                if args.unet == True:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.save_ckpt_step == 0:
                        ...
                        if args.clip == True:
                            ...
                            torch.save(clip.state_dict(), os.path.join(save_path, f"clip_image_encoder_{epoch}.pth"))
                        if args.unet == True:
        ...
        if accelerator.is_main_process:
            ...
            if args.clip == True:
                clip = accelerator.unwrap_model(clip)
                torch.save(clip.state_dict(), f"./training/clip_image_encoder_{epoch}.pth")
            if args.unet == True:
                diffusion = accelerator.unwrap_model(diffusion)
...
def main(args):
    ...
    if args.use_lr_scheduler:
        ...
        lr_scheduler = CosineAnnealingWarmUpRestarts(optimizer, T_0=80000, T_mult=1, eta_max=args.lr,  T_up=20, gamma=1)
    else:
        optimizer = AdamW(
                params_to_optimize,
                lr=args.lr,
